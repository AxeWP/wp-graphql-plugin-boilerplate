...
- feat: Add `package.json` with `wp-env` scripts, Prettier config, `.nvmrc`, and `.editorconfig`.
- feat: Replace Codeception/Docker test harness with PHPUnit + `@wordpress/env`. Tests now live in `tests/phpunit/` and run via `npm run test:php`.
- fix: Register plugin settings on `admin_init` instead of `init`.
- fix: Actually bootstrap `Main::instance()` in `graphql_pb_init()`. H/t @natac13
- fix: Activation/deactivation callbacks now run directly instead of returning a closure.
- chore: Remove `bin/`, `docker/`, `docker-compose.yml`, `c3.php` coverage hook, and Codeception suite configs.
- chore: Simplify composer scripts to `lint`/`format`/`test`; update deps.
- ci: Replace the four code-quality/standard/integration workflows with a single `ci.yml`; add Dependabot config.
